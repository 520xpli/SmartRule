...
<body>
	<div id="treeContainer">
		<div dojoType="dojo.data.ItemFileReadStore" jsId="menuContinentStore"
			url="<bean:write name='stSingleRuleForm' property='treeUrl'/>"></div>
		<div dojoType="dijit.tree.ForestStoreModel" jsId="menuContinentModel"
			store="menuContinentStore" rootId="identifier" rootLabel='产品选择'
		...
		<div dojoType="dijit.Tree" id="menuTree" model="menuContinentModel"
			...
			function clickTreeNode(item,node,e){
				if(window.opener){
					window.opener.callback(item.code, item.name, item.parentcode, item.parentname);
				}
				window.top.close();
			}
		</script>
