...
<BODY>
	<h1 style="text-align: center; font-size: 18px">准入规则<font onclick="window.parent.isShowCode=false">维</font>护</h1>
		...
		<div class="position" style="display:none">
			<div id="backupTable">
				<table style="text-align: center;margin-top:15px" class="ruletable grid">
					...
					<tr>
						<td>具体公式:</td>
						<td colspan="4" id="formulaDetail" style="word-wrap:break-word;word-break:break-all;white-space:normal;width:auto;"></td>
						<td>							
						<logic:equal value="1" name="stSingleRuleForm" property="strucVo.classification">
							结果:
							</logic:equal>
							<logic:equal value="2" name="stSingleRuleForm" property="strucVo.classification">
							风险程度:
							</logic:equal></td>
						<td>
						<logic:equal value="1" name="stSingleRuleForm" property="strucVo.classification">
						<select name="strucVo.detailList.ruleresult">
								<option>请选择</option>
					...
					<table style="text-align: center;margin-top:10px;" class="ruletable grid">
						...
						<tr>
							<td style="width:10%" class="displayText">具体公式:</td>
							<td colspan="4" id="formulaDetail" style="word-wrap:break-word;word-break:break-all;white-space:normal;width:auto;">
							</td>
							...
							<logic:equal value="2" name="stSingleRuleForm" property="strucVo.classification">
								<select name="strucVo.detailList.ruleresult">
									<option value="">请选择</option>
									<option value="0" <logic:equal value="0" name="detail" property="ruleresult"> selected</logic:equal> >无</option>
									<option value="1" <logic:equal value="1" name="detail" property="ruleresult"> selected</logic:equal> >低</option>
									<option value="2" <logic:equal value="2" name="detail" property="ruleresult"> selected</logic:equal> >中</option>
									<option value="4" <logic:equal value="4" name="detail" property="ruleresult"> selected</logic:equal> >高</option>
							</select>
							</logic:equal>
							</td>
						</tr>
...
<script type="text/javascript">
	...
	function commit(){
		...
		$.ajax({
			...
			url:"<%=request.getContextPath()%>/StRuleMaintain.do?operAtt=insert",
	...
	function getPropertyData(){
		...
		if(objectcode != null && objectcode != ''){
			$.ajax({
				...
				url:"<%=request.getContextPath()%>/StRuleMaintain.do?operAtt=queryProerty",
				success:function(data){
					...
					$.ajax({
						type:"POST",
						datatype:"json",
						url:"<%=request.getContextPath()%>/StRuleMaintain.do?operAtt=queryProduct",
	...
	function popupProduct(p){
		var modelcode = $("input[name='modelcode']").val();
		var treeUrl = "<%=request.getContextPath()%>/StRuleMaintain.do?operAtt=queryProduct&modelcode=" + modelcode;
		var url = "<%=request.getContextPath()%>" + "/StRuleMaintain.do?operAtt=popupProduct&modelcode=" + modelcode + "&treeUrl=" + treeUrl;
